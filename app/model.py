...
# assign lables to the dataset with agglomerative clustering method.
def agg(file_path):
...
def find_frequency(df,col, l=list(range(70)), p=list(range(26))): # predefine lottery numbers and powerball number range, 70 and 26
    ...
    P_fre = []
    while k < len(p):
        pfr = df.loc[df[col[len(col)-1]]==k].value_counts().sum() # loop through powerball array and find all numbers equal to that value, aggregate them
        P_fre.append(pfr) # all powerball freqency
        k += 1
    print("powerball freqency.....",P_fre)

    return Frequency,P_fre
# use the frequency, to find which numbers have the high frequencey in the an array
def find_max(freq_arr,number_arr): # arr is the frequency
    ind = []
    val = max(freq_arr)
...
if __name__=="__main__":
    ...
    df, df_class = agg(file_path=file_path)
    
"""     
    # use highest frequency for the selected numbers.
    file = "../data/selected.csv"
    df = pd.read_csv(file)
    col = ['N1','N2','N3','N4','N5','Powerball']
    l=list(range(70))
    p=list(range(26))
    Frequency,P_fre = find_frequency(df,col, l=list(range(70)), p=list(range(26)))
    final= []
    for i in range(len(Frequency)):
        number_freq = find_max(Frequency[i],l)
        final.append(number_freq)
    print("most frequent number in all selected lottery are - ", final)
    power_feq = find_max(P_fre,p)

    
    print("most frequent powerball in selected lottery: ", power_feq) """